...
import io.github.theepicblock.polymc.mixins.ChunkFilterAccessor;
import io.github.theepicblock.polymc.mixins.TSCSAccessor;
...
public abstract class FixLighting {
    ...
    private List<ServerPlayerEntity> onGetPlayersWatchingChunk(ChunkHolder.PlayersWatchingChunkProvider chunkHolder, ChunkPos chunkPos, boolean onlyOnWatchDistanceEdge) {
        ...
        var watchDistance = ((TSCSAccessor)this.playersWatchingChunkProvider).getWatchDistance();
        ...
        return watchers.stream()
                .filter(watcher -> {
                    ...
                    var isOnEdge = ChunkFilterAccessor.callIsWithinDistanceExcludingEdge(this.getPos().x, this.getPos().z, watcherChunk.getSectionX(), watcherChunk.getSectionZ(), watchDistance);
